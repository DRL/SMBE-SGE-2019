...
Metrics for &pi;, &D</sub>(xy); and &F</sub>(st); are based on all non-FGV blocks in the dataset. The reason for &pi; of a population being lower than its heterozygosity, resides in the fact that mutuples of well covered, conserved (i.e. monomorphic) blocks contribute more than those of poorly covered ones.
...
- ```*.fst_genome_scan.png``` : a &F</sub>(st); scan across chr18
- ```*.pi_genome_scan.png``` : a &pi; scan across chr18
- ```*.pi_scatter.png``` : &pi; vs. &D</sub>(xy); scatter plots of all windows in chr18
